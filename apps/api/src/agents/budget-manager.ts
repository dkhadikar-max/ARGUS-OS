...
// denormalizeCost/allocate). No consume()/evaluateReallocation()/
// estimateCapabilityCost()/getCostBreakdown() yet -- those need a live
// Controller actually spending a budget mid-decision, which doesn't exist
// (Planning Policy / Controller is the next phase in the migration order).
// Nothing here is wired into orchestrator.ts or decision.service.ts; these
// are pure functions over the real RawCost/DecisionObjectiveValue shapes
// decision-state.ts (Phase 1) already established.
