...
| Policy v2.1 "Three Entry Paths" | CTASection: Free Decision Assessment ($0) / Intelligence Sprint ($7,500) / Enterprise Engagement ($25,000+) |
...
- **The $7,500 figure turned out to be real policy, just renamed.** The Policy's "What We Killed" list retires the *name* "Revenue Sprint" in favor of "Intelligence Sprint" — same $7,500 price point, now one of three explicitly-priced, frozen GTM entry paths (Free Decision Assessment $0 / Intelligence Sprint $7,500 / Enterprise Engagement $25,000+), not a fabrication. `CTASection.tsx` now shows these three paths (name, price, target company size, and the sales-qualification signal each one represents), reusing the exact same bordered-grid-of-cards pattern as before (3 columns instead of 4) — this is a policy update superseding the same-day SaaS-tier CTA, not a reversal of the earlier audit finding, which was correct given what was known at the time. Dollar amounts are shown here deliberately, unlike the Bible's SaaS subscription tiers: the Policy marks these three prices "Do Not Change," a different, later, and more specific instruction than the earlier one withholding the subscription numbers.
- **Added — the frozen "Homepage Hierarchy"** (Pain → Promise → Proof → Product → Path, explicitly "Do Not Change"): `HeroSection.tsx` now leads with the Pain stat ("40% of pipeline time is wasted on poor-fit prospects"), states the Promise verbatim ("Know why before you act — every time"), keeps the existing sidebar mockup as Proof and the existing H1 as Product framing (neither contradicted the Policy, so neither was rewritten), and the two Path buttons are now "Free Assessment" / "Intelligence Sprint" (was "Start Free" / "View the Evidence Chain" — the Evidence Chain link isn't lost, it's still in the nav bar).
...
- No demo request form or gated content — the three entry paths and their prices are shown directly (Policy v2.1)
...
- Bible §13.2's SaaS subscription tiers (Free/Starter/Pro/Enterprise) are no longer shown anywhere on this page — the homepage's CTA slot is now the Policy's Three Entry Paths instead (see "Policy v2.1 alignment" above). The subscription tiers are still the real product plans a customer lands on after entering through one of those paths; they just don't have a dedicated slot on this single-page site right now. Worth a `/pricing` sub-page once this site grows beyond one page.
